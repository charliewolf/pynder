...
class ProfileDescriptor(object):
 ...
 def __set__(self, instance, value):
  ...
  profile['interested_in']=map(lambda x: constants.GENDER_MAP_REVERSE[x], profile['interested_in'])
...
class InterestedInDescriptor(ProfileDescriptor):
 # makes gender human readable
 def __get__(self, instance, owner):
  interested_in=super(InterestedInDescriptor,self).__get__(instance,owner)
  return map(lambda x: constants.GENDER_MAP[x], interested_in)
 def __set__(self,instance,value):
  interested_in=map(lambda x: constants.GENDER_MAP_REVERSE[x], value)
  return super(InterestedInDescriptor,self).__get__(instance,interested_in)
